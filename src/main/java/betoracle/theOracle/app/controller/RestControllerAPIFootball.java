...
import betoracle.theOracle.app.object.Status;
...
public class RestControllerAPIFootball {
	...
	/**
	 * Available fixtures status SHORT LONG TYPE DESCRIPTION: 
	 ...
	 * PEN Match
	 * Finished After Penalty Finished Finished after the penalty shootout
	 *  PST Match
	 * Postponed Postponed Postponed to another day, once the new date and time is
	 * known the status will change to Not Started 
	 ...
	 * WO WalkOver Not Played Victory by forfeit or absence of competitor LIVE In Progress In Play Used in very rare cases. It indicates a fixture in progress but the data indicating the half-time or elapsed time are not available
	 * 
	 ...
	 **/
	@GetMapping("/apifootballv1/fixturesByStatus/{league}/{season}/{status}")
	@ResponseBody
	String fixturesByStatus(@PathVariable String league, @PathVariable String season, @PathVariable String status) {

		HttpRequest request = HttpRequest.newBuilder()
				.uri(URI.create("https://api-football-v1.p.rapidapi.com/v3/fixtures?league=" + league + "&season="
						+ season + "&status=" + status))
				.header("content-type", "application/octet-stream")
				.header("X-RapidAPI-Key", "38446dcaecmsh727d79551bbd0cdp1b2df3jsnb547de9aa069")
	...
	String predictions(@PathVariable String fixtureNumber) {
		...
		if (status.getCurrent() < (status.getLimitDay() - 10)) {
			...
			return "sono rimaste solo 10 chiamate, vuoi farle lo stesso?";
		}
